...
class ElectronPage extends BrowserPage {
  ...
  _waitForDialog(dialogType, action, message = undefined, isClickOK = false){
    return new Promise((resolve, reject)=>{
      ...
      let timeoutId = setTimeout(()=>{
        this.socket.removeListener(channel, dialogCallback);
  ...
  waitForSelector(cssSelector){
    return new Promise((resolve, reject)=>{
      const channel = 'find-selector-synchronous';

      this.socket.once(channel, (arg)=>{
        if(arg == 'found'){
          resolve();
        }else{
          reject(new Error(arg == 'timeout' ? 'timeout' : 'unknown error'));
        }
      });
