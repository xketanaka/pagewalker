...
/**
 * BrowserPage implementation for Electron.
 */
class ElectronPage extends BrowserPage {
    ...
    return new Promise((resolve, reject)=>{
      ...
      this.nativeObject.session.webRequest.onResponseStarted((details)=>{
        ...
        if (disposition.match(/[\s]*attachment/)) {
          ...
          const matched1 = disposition.match(/filename\*=(?:utf|UTF)-?8''(.*)$/);
          const matched2 = disposition.match(/filename=\"(.*)\"$/);
          if(matched1) {
            matched1[1] = decodeURIComponent(matched1[1]);
          }
          this.nativeObject.once('did-navigate', (event, url, httpResponseCode) => {
            clearTimeout(timeoutId);
            resolve({ filename: (matched1 ? matched1[1] : (matched2 ? matched2[1] : "no name")), saveedFilePaht: '' });
          });
        }
      });
  ...
  _waitForDialog(dialogType, action, message = undefined, isClickOK = false){
    return new Promise((resolve, reject)=>{
      const channel = 'dialog_' + randomUUID();
  ...
  setExtraHTTPHeaders(headers) {
    ...
    this.nativeObject.session.webRequest.onBeforeSendHeaders(filter, (details, callback) => {
      for (const key in this.extraHTTPHeaders) {
...
/**
 * BrowserSocket implementation for Electron (based on ipcMain).
 */
