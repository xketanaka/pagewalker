...
class PlaywrightPage extends BrowserPage {
  ...
  off(eventName, callback){
    if(eventName in PlaywrightPage.EventMappings){
      this.nativeObject.off(PlaywrightPage.EventMappings[eventName], callback)
    }else{
  ...
  waitForDownload(action, options){
    ...
    return new Promise((resolve, reject)=>{
      timeoutId = setTimeout(()=>{ cleanup() && reject(new Error('timeout')) }, options.timeout || TIMEOUT_MSEC);
      ...
      callbacks['download'] = (download)=>{
        ...
        .then((downloadedPath)=>{
          if(options.downloadPath) {
            const savedFilePath = path.resolve(path.join(options.downloadPath, filename));
            return fs.promises.mkdir(options.downloadPath, { recursive: true })
              .then(_ => fs.promises.copyFile(downloadedPath, savedFilePath))
              .then(_ => savedFilePath);
          }else{
            return downloadedPath;
          }
        })
        .then((savedFilePath)=>{ cleanup() && resolve({ filename: filename, savedFilePath: savedFilePath }) })
