...
class PlaywrightBrowser extends Browser {
  constructor(options = {}){
    ...
    const { browserType = 'chromium', viewport, contextOptions, ...launchOptions } = options;

    playwright[browserType].launch(launchOptions)
    .then((browser)=>{
      this._browser = browser;
      return browser.newContext(Object.assign({ viewport: viewport, acceptDownloads: true }, contextOptions));
    })
    .then((context)=>{
