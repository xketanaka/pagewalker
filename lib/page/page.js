...
class Page {
  ...
  load(url, options = {}){
    ...
    if(this.window.isReadyForPage()){
      ...
      return new Promise((resolve, reject)=>{
        this.window.once('readyForPage', ()=>{ loadURL().then(resolve).catch(reject) });
      });
    }
  }
  /**
  ...
  waitForNewWindow(action){
    return this.invokeAfterReady((resolve, reject)=>{
      let timeoutId = setTimeout(()=>{ reject('timeout in waitForNewWindow') }, TIMEOUT_MSEC);
      this.browserPage.removeAllListeners(BrowserPage.Events.NewWindow);
      pageWalker.browser.once(Browser.Events.NewWindow, (browserWindow)=>{
        clearTimeout(timeoutId);
   ...
   * @private
   */
  invokeAfterReady(func, tryTimes = 0){
    return new Promise((resolve, reject)=>{
      if(!this.window.isReadyForPage()){
  ...
  executeJs(code){
    return this.invokeAfterReady((resolve, reject)=>{
      return this.browserPage.executeJavaScript(code, true).then(resolve);
    })
  }
  /**
