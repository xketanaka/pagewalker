...
class Page {
  ...
  load(url, options = {}){
    let loadURLFunction = ()=>{
      return this.waitForPageLoad(()=>{ if(url) this.webContents.loadURL(url, options) })
    };
  ...
  waitForPageLoad(action){
    return this._waitForContent(action, 'did-finish-load', 'did-fail-load')
  }
  /**
  ...
  waitForFrameLoaded(action){
    return this._waitForContent(action, 'did-frame-finish-load', 'did-fail-load')
  }
  /**
   * @private
   */
   ...
   * @return {Promise}
   * @example
   *   const filePath = await page.waitForDownload(()=>{
   *     ...
   *   })
  ...
  waitForNewWindow(action){
    return this.invokeAfterReady((resolve, reject)=>{
      let timeoutId = setTimeout(()=>{ reject('timeout') }, TIMEOUT_MSEC);
      this.webContents.removeAllListeners('new-window');
   ...
   * @return {Promise}
   */
  waitForIpcReceive(action, channel = 'asynchronous-message'){
    return this.invokeAfterReady((resolve, reject)=>{
      let timeoutId = setTimeout(()=>{ reject('timeout') }, TIMEOUT_MSEC);
