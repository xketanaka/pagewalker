...
/**
 * User-facing object representing one page in the browser.
 * Provides page load, element finder, wait helpers, dialogs, downloads and screenshots.
 */
class Page {
  ...
  load(url, options = {}){
    ...
    return this.window.whenReady().then(()=> loadURL(url));
  }
  /**
   * Wait loading for contents in new page.
   * @param {function} action - A function representing the operation of loading new page.
   * @param {object} options
   * @param {number} options.timeout - msec. give up waiting after this (default config.mocha.timeout).
   *   Specify it on a scenario which expects the wait to time out, so that it does not wait long.
   * @return {Promise}
   */
  waitForPageLoad(action, options = {}){
    if(this._context){
      return this.waitForBrowserSocket({ channel: 'iframe-page-load', timeout: options.timeout }, ()=>{
    ...
    return this.newPromiseWithCheckReady((resolve, reject)=>{
      let timeoutId = setTimeout(()=>{ reject(new Error('timeout in waitForPageLoad')) }, options.timeout || TIMEOUT_MSEC);
  ...
  /**
   * Set callback to next page event, and returns an unset callback function ('unset' means "unsubscribe").
   * It is used to subscribe to the next page load (navigation) event once.
   * @param {Function} callback - called once when the page fires a load event
   * @return {Function} unsubscribe function
   * @private
   */
  onceLoad(callback){
    this.browserPage.once(BrowserPage.Events.Load, callback);
    return ()=>{ this.browserPage.off(BrowserPage.Events.Load, callback); };
  }
  /**
   ...
   * @param {string} options.downloadPath - directory to save the file in (default config.fileDownloadDir)
   * @param {number} options.timeout - msec. give up waiting after this (default config.mocha.timeout)
   ...
   * newWin.close();
   */
  waitForNewWindow(action, options = {}){
    return this.newPromiseWithCheckReady((resolve, reject)=>{
   ...
   * @return {Promise}
   */
  waitForSelector(cssSelector, action, options = {}){
    return this.waitForFinder(this.find(cssSelector), action, options);
  }
  /**
   ...
   * @return {Promise}
   */
  waitForFinder(finder, action, options = {}){
    const interval = (config.autoWaiting && config.autoWaiting.interval) || 100;
    const timeout = options.timeout || TIMEOUT_MSEC;
    return this.newPromiseWithCheckReady((resolve, reject)=>{
      // Retry target element to be found, If timeout, Error('timeout')
      finder.withRetry({ timeout, interval }).evaluate().then(resolve, reject);
    }, action);
  }
  /**
   * @param {object} socketOption - specify channel property, and timeout property(msec. default config.mocha.timeout).
  ...
  /**
   * @param {object} confirmOption - specify message property(default undefined), isClickOK property(default true),
   *   and timeout property(msec. default config.mocha.timeout)
   * @param {function} action - A function representing the operation of open confirm dialog
   * @return {Promise}
   * @example
   * await page.waitConfirm({ message: "Are you OK?", isClickOK: false }, ()=>{
  ...
  waitForConfirm(confirmOption, action){
    ...
    return this.newPromiseWithCheckReady((resolve, reject)=>{
      this.browserPage.waitForConfirm(action, { message, isClickOK, timeout: confirmOption.timeout })
  ...
  waitForAlert(alertOptioin, action){
    ...
    return this.newPromiseWithCheckReady((resolve, reject)=>{
      this.browserPage.waitForAlert(action, { message: alertOptioin.message, timeout: alertOptioin.timeout })
      .then(resolve).catch(reject);
    })
  }
  /**
  ...
  newPromiseWithCheckReady(func, action){
    // An error created in a timer callback (e.g. reject on timeout) has no trace of the scenario line,
    // Capture the caller's stack here, and append it to the error thrown from this promise.
    const callerError = new Error();
    Error.captureStackTrace(callerError, Page.prototype.newPromiseWithCheckReady);
    const withCallerStack = (err)=>{
      if(!(err instanceof Error)) err = new Error(err);
      if(callerError.stack && err.stack && !err.stack.includes(callerError.stack.split("\n")[1] || " ")){
        err.stack = err.stack + "\n" + callerError.stack.replace(/^Error\n?/, "");
      }
      throw err;
    };
    ...
    return Promise.all([promise1, action && promise2])
      .then(values => values[0]).catch(withCallerStack);
  }
  /**
  ...
  /**
   * Assert that the current screen is equal to the expected screenshot.
   * The screenshot is compared against "(screenshotsDir)/expected/(browserVariant)/(identifier).png",
   *
   * @param {String} identifier - identifier used as the screenshot fileName
   * @param {object} options - overrides config.assertion (ssimThreshold, diffThreshold)
