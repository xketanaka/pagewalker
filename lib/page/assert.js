...
class Assert {
  ...
  equal(identifier, options){
    ...
    .then((images)=>{
      ...
      const diffFilePath = path.join(this._config.screenshotsDir, "diff.png");
      return new Promise((resolve, _)=>{
        diff.pack().pipe(fs.createWriteStream(diffFilePath)).on('finish', resolve);
      })
      .then(()=>{
        let indent = '\t\t\t'
        throw new Error(
`[ERR_ASSERTION]: "actual/${variant}/${identifier}" is different to "expected/${variant}/${identifier}".
${indent}Saved the difference between the two images As "${diffFilePath}"`
        );
      })
    })
  }
