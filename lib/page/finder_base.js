...
class FinderBase {
  ...
  toJsCode(options = {}){
    ...
    const toJsString = (str => JSON.stringify(String(str)));

    let findCode = `
      let _document = ${this.context ? this.context : 'window'}.document;
      var elements = Array.from(_document.querySelectorAll(${toJsString(conditions[0].selector)}));
    `;
    findCode += conditions.slice(1).map((condition)=>{
      if(condition instanceof Selector)
        return `{
          elements = elements.reduce((arr, e, idx)=>{
            return arr.concat(Array.from(e.querySelectorAll(${toJsString(condition.selector)})));
          }, []);
        }`;
   ...
   * When config.slowMotion is over 0, wait that msec before running the action.
   * @return {Promise} Promise resolved in evaluated JavaScript code
   */
  evaluateAction(){
    const slowMotion = Number((config && config.slowMotion) || 0);
    if(slowMotion > 0){
      return new Promise((resolve)=> setTimeout(resolve, slowMotion)).then(()=> this._evaluateAction());
    }
    return this._evaluateAction();
  }
  /**
   * @private
   * @return {Promise} Promise resolved in evaluated JavaScript code
