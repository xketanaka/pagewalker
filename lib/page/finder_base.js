...
class FinderBase {
   ...
   *   'marker' return the not-ready marker. In auto-waiting mode, the caller can retry.
   * @return {string}
   */
  toJsCode(options = {}){
    ...
    if(notFoundBehavior === 'marker'){
      // auto-waiting attempt: narrow down to actionable elements, and return the marker
      jsCode += `
    ...
    else if(notFoundBehavior === 'throw'){
      jsCode += `
        if(elements.length == 0){
  ...
  evaluateAction(){
    ...
    if(!autoWaiting.enabled || this._noWait){
      return this._evaluate({ notFoundBehavior: 'throw' });
    }
    ...
    const delay = ()=>{ return new Promise(resolve => setTimeout(resolve, interval)) };

    const attempt = ()=>{
      return this._evaluate({ notFoundBehavior: 'marker' })
      .then((result)=>{
        if(result !== NOT_READY_MARKER) return result;
        if(Date.now() >= deadline){
          throw new Error(`Element not found or not actionable within ${timeout}ms`);
        }
        return delay().then(attempt);
      }, (err)=>{
        // the execution context can be destroyed by a navigation between attempts. retry in that case.
        const retriable = /Execution context was destroyed|Cannot find context|because of a navigation|frame got detached/i.test(String(err && err.message));
        if(Date.now() < deadline && retriable){
          return delay().then(attempt);
        }
        throw err;
      });
  ...
  /**
   * Evaluate this finder and resolve with the result. If no element matches, does not throw even.
   * @return {Promise} Promise resolved in evaluated JavaScript code
   */
  evaluate(){
