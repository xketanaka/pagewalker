...
class Finder extends FinderBase {
   ...
   * Pass the returned selector to withMarkerCleanup() to remove it after use.
   * @private
  ...
  /**
   * Run the given promise, then remove the marker before resolving or rejecting.
   * Note: Promise.prototype.finally does not wait for the promise its callback returns,
   * so the cleanup must be chained explicitly like this.
   * @private
   * @param {string} selector - the selector returned by markFirstElement()
   * @param {Promise} promise - the native operation using the marker
   * @return {Promise}
   */
  withMarkerCleanup(selector, promise){
    // failures are ignored: the page may be gone already, and a leftover marker is harmless
    // because its value is unique per call
    const removeMarker = ()=> this.page.executeJs(
      `document.querySelectorAll('${selector}').forEach((e)=>{ e.removeAttribute("${MARKER_ATTRIBUTE}") })`
    ).catch(()=>{ /* ignore */ });

    return promise.then(
      (result)=> removeMarker().then(()=> result),
      (err)=> removeMarker().then(()=>{ throw err })
    );
  }
  /**
