...
class Finder extends FinderBase {
  ...
  /**
   * @example
   * page.find("input").some(PageFilter.haveValue("123"))
   */
  some(code){
    return this.withMatcher(code, `(filtered)=>{ return filtered.length != 0 }`)
  }
  /**
   * @example
   * page.find("input").none(PageFilter.haveValue("123"))
   */
  none(code){
    return this.withMatcher(code, `(filtered)=>{ return elements.length != 0 && filtered.length == 0 }`)
  }
