...
class FinderAction {
  ...
  static setValue(val){
    return `(elements)=>{
      for(let i = 0; i < elements.length; i++){
        let beforeVal = elements[i].value;
         elements[i].value = "${val}"
         if (beforeVal != elements[i].value) {
   ...
   */
  keydown(options){
    return this.withAction(FinderAction.keydown(options)).allowNotFound(false).evaluate()
  }
  /**
   ...
   */
  submit(){
    return this.withAction(FinderAction.submit).allowNotFound(false).evaluate();
  }
}
