...
const DebugSession = require('./walker/debug_session');
...
class PageWalker {
  ...
  start(config){
    // it opens the inspector before initialize(), which loads the scenario files
    this._debugSession = DebugSession.create(config);
    this.initialize(config);

    // Attach event handler to browser.
    this._browser.on(Browser.Events.Ready, ()=>{
      this._debugSession.waitUntilResumed()
      .then(()=>{
