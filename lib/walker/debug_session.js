...
class DebugSession {
  ...
  waitUntilResumed(){
    ...
    .then(()=>{
      console.log(`[pagewalker] Debug mode: the debugger is listening on ${inspector.url()}`);
      console.log(`[pagewalker] Waiting for DevTools to connect. The execution is then paused,`);
      console.log(`[pagewalker] so open your scenario file on DevTools, set breakpoints and resume it.`);

      // DevTools sends "Runtime.runIfWaitingForDebugger" once on connecting, which releases this.
      // Therefore this must be called before DevTools connects, not after.
  ...
  launchBrowser(){
    ...
    .then(([executablePath, port])=>{
      ...
      // Wait for the browser while the event loop is running. waitForConnection() blocks it,
      ...
      .then(()=>{
        // Show DevTools by another invocation with the same user-data-dir, which just passes the
        // url to the browser launched above. It must be done just before blocking the event loop,
        // because waitForDebugger() is released only by DevTools connecting after it is called.
        const url = `http://127.0.0.1:${port}/devtools/js_app.html?ws=${inspector.url().replace(/^ws:\/\//, '')}`;
        spawn(executablePath, commonArgs.concat([url]), { stdio: 'ignore' });
      });
    });
  }
  /**
