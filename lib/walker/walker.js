...
class Walker {
  ...
  loadFiles(){
    ...
    this.mocha.suite.on('post-require', (context)=>{
      ...
      // object property function support is describe("title", { "title": function(){...} }, (fn)=>{ fn() })
      originalFunctions.describe = context.context = context.suite = context.describe = Walker.objectPropertySupport(originalFunctions.describe);
      originalFunctions.xdescribe = context.xcontext = context.xdescribe = Walker.objectPropertySupport(originalFunctions.xdescribe);
      originalFunctions.describeOnly = context.describe.only = Walker.objectPropertySupport(originalFunctions.describeOnly);
    })
  ...
  static objectPropertySupport(originalFunc){
    let newFunc = function(title, fn, callback){
      if(typeof fn == "object"){
      ...
      originalFunc.call(this, title, (typeof fn == "object")? ()=>{ callback(fn[title]) }: fn);
    }
