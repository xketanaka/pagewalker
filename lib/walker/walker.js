...
const FsExt = require("../utils/fs_ext");

class Walker {
  constructor(config){
    this.config = config;
    this.mocha = new Mocha(Object.assign({}, config.mocha, { timeout: config.mocha * 2 });

    if(this.config.screenshotsDir){
      FsExt.mkdirpSync(path.join(this.config.screenshotsDir, 'actual'));
      FsExt.mkdirpSync(path.join(this.config.screenshotsDir, 'expected'));
    }
